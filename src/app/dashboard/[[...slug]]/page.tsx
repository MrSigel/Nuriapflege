...
export default async function CompanyDashboardPage({ params }: PageProps) {
  ...
  return (
    ...
    >
      ...
      {careDocs ? <CareDocumentationPage data={careDocs} actions={{ createCareDoc, updateCareDoc, reviewCareDoc }} /> : null}
      {billing ? <BillingPage data={billing} actions={{ createBillingItem, updateBillingItem, changeBillingItemStatus, createInvoice, updateInvoiceStatus }} /> : null}
      {documents ? <DocumentsPage data={documents} actions={{ uploadDocument, updateDocument, archiveDocument, deleteDocument, getDocumentSignedUrl }} /> : null}
      {communication ? <CommunicationPage data={communication} actions={{ createDirectConversation, createGroupConversation, sendMessage, markConversationRead, archiveConversation, addParticipant, removeParticipant }} /> : null}
      {applicants ? <ApplicantsPage data={applicants} actions={{ createApplicant, updateApplicant, changeApplicantStatus, changeApplicantRating, setApplicantFollowUp }} /> : null}
      {websiteOnline ? <WebsiteOnlinePage data={websiteOnline} actions={{ createWebsiteLead, updateWebsiteLead, changeWebsiteLeadStatus, changeWebsiteLeadPriority, setWebsiteLeadFollowUp, createOnlineTask, updateOnlineTask, changeOnlineTaskStatus, changeOnlineTaskPriority }} /> : null}
      {qmMd ? <QmMdPage data={qmMd} actions={{ createQmItem, updateQmItem, changeQmItemStatus, changeQmItemPriority, setQmItemDueDate, createQmMeasure, updateQmMeasure, changeQmMeasureStatus, changeQmMeasurePriority, createQmEvidence }} /> : null}
      {exports ? <ExportsPage data={exports} actions={{ createExportJob, deleteExportJob }} /> : null}
      {activityLogs ? <ActivityLogsPage data={activityLogs} /> : null}
      {compliance ? <CompliancePage data={compliance} actions={{ createComplianceItem, updateComplianceItem, changeComplianceItemStatus, changeComplianceItemPriority, createComplianceEvidence }} /> : null}
      {payment ? <PaymentPage data={payment} actions={{ markPaymentSent, saveBillingData }} /> : null}
      {clients ? <ClientsPage data={clients} actions={{ createClient, updateClient, changeClientStatus }} /> : null}
      {shifts ? <ShiftsPage data={shifts} actions={{ createShift, updateShift, changeShiftStatus }} /> : null}
      {timeTracking ? <TimeTrackingPage data={timeTracking} actions={{ createTimeEntry, updateTimeEntry, reviewTimeEntry }} /> : null}
      {tours ? (
        <ToursPage
          data={tours}
          actions={{ createTour, updateTour, changeTourStatus, upsertTourStop, deleteTourStop, moveTourStop }}
        />
      ) : null}
      {employees ? (
        <EmployeesPage
      ...
      {rolesPermissions ? <RolesPermissionsPage data={rolesPermissions} saveAction={saveRolePermissions} /> : null}
      {settings ? <SettingsPage data={settings} saveAction={saveSettings} /> : null}
      {!overview && !locations && !careDocs && !billing && !documents && !communication && !applicants && !websiteOnline && !qmMd && !exports && !activityLogs && !compliance && !payment && !clients && !shifts && !timeTracking && !tours && !employees && !rolesPermissions && !settings ? <DashboardPage route={route} context="company" /> : null}
    </DashboardShell>
  );
}
